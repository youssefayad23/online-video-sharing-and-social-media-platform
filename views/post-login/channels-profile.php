...
$index1 = intval($_GET['channelID']);
$_SESSION['channelID'] = $_GET['channelID'];
$row = $channel->getChannelInfo($index1, $con);
$channelName = $row['channelName'];
$channelID = $row['channelID'];



if (!isset($_GET['videoGetter']) && !isset($_GET['channelIndex'])) {
?>
                    ...
                    <nav class="navbar navbar-expand-lg navbar-light">
                        ...
                        <div class="collapse navbar-collapse" id="navbarSupportedContent">
                            <ul class="navbar-nav mr-auto">
                                <li class="nav-item active">
                                    <a class="nav-link" href="channels-profile.php?channelID=<?php echo $channelID; ?>">Videos <span class="sr-only">(current)</span></a>
                                </li>

                                <li class="nav-item">
                                    <a class="nav-link" href="about-page.php?channelID=<?php echo $channelID; ?>">About</a>
                                </li>
                ...
                <div class="container-fluid">
                    <div class="video-block section-padding">
                        <div class="row">
                            ...
                            <?php
                            $query = mysqli_query($con, "SELECT * FROM video WHERE  channelID='$index1'  ");
                            displayVideos($query);
                            ?>
...
<?php
} elseif (isset($_GET['videoIndex'])) {
    $index = $_GET['videoIndex'];
    header("location: video-page.php?videoIndex=" . $index . "");
} else {
    $index = $_GET['channelIndex'];
    $userid = $user->getUserID($_SESSION['email'], $con);
    $result3 = mysqli_query($con, "SELECT * FROM subscribedto WHERE userID='$userid' AND channelID='$index'");
    if ($result3->num_rows > 0) {
        $channel->unsubscribe($userid, $index, $con);
        header("location: channels-profile.php?channelID=" . $index . "");
    } else {
        $channel->subscribe($userid, $index, $con);
        echo $userid . "  " . $index;
        header("location: channels-profile.php?channelID=" . $index . "");
    }
}
