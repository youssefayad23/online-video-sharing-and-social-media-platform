...
$fetchVideos = mysqli_query($con, "SELECT * FROM playlistvideoids where playlistID = $index");
$row1 = mysqli_fetch_assoc($fetchVideos);
$videoID = $_SESSION['videoID'];
$fetchVideos2 = mysqli_query($con, "SELECT * FROM video where videoID =  $videoID ");
...
if (isset($_POST['addComment']) && $_POST['comment'] != '') {
   ...
   $result = mysqli_query($con, $sql);
   header("location: video-pageForplaylist.php?playlistIndex=" . $_SESSION['playlistID'] . "&&videoID=" . $index1 . "");
}


if (isset($_POST['like'])) {
   $sessionEmail = $_SESSION['email'];
   $result2 = mysqli_query($con, "SELECT * FROM user WHERE email='$sessionEmail'  ");
   $row2 = mysqli_fetch_array($result2);
   $userid = $row2['userID'];
   $result3 = mysqli_query($con, "SELECT * FROM likedvideos WHERE userID='$userid' AND videoID='$videoID'");
   if ($result3->num_rows > 0) {

      $video->unlike($userid, $index1, $con);

      header("location: video-pageForplaylist.php?playlistIndex=" . $_SESSION['playlistID'] . "&&videoID=" . $videoID . "");
   } else {
                     ...
                     <div class="col-md-8">
                        <div class="single-video-left">
                           <div class="single-video">
                              ...
                              <video width="100%" height="315" src=" <?php echo $location; ?>" controls frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></video>
                        ...
                        <div class="single-video-right">
                           ...
                           while ($row1 = mysqli_fetch_assoc($fetchVideos)) {
                              ...
                              echo "
                                       ...
                                       <a  href='video-pageForplaylist.php?playlistIndex=" . $_SESSION['playlistID'] . "&&videoID=" . $videoID . "'><video class='img-fluid' style='height:200px; width: 350px;padding-right:10px;' src='" . $location . "'
                                             ></video></a><br>
                                             <a style='text-decoration: none; color: black;' href='video-pageForplaylist.php?playlistIndex=" . $_SESSION['playlistID'] . "&&videoID=" . $videoID . "'><p  style='display:inline;'>" . $name . "</p></a>
                           ";
                           }
...
<?php
   ...
   if ($result3->num_rows > 0) {
      $sql = "DELETE FROM subscribedto WHERE userID='$userid' AND channelID='$index'";
